...
function rapePickForeplay(part) {
    const scenes = RAPE_FOREPLAY_SCENES[part];
...
const PRIVATE_PART_TYPES = [
    ...
    { name: '玉蚌穴', adj: '玉蚌含珠的', desc: '两瓣紧紧闭合如蚌壳，色泽粉嫩' },
...
function _getSizeDesc(bust) {
    ...
    if (bust >= 34) return ['饱满鼓胀的', '一双丰盈的', '两只乳鸽般鼓胀的'];
...
const RAPE_SEX_POSITIONS = {
    ...
    doggy: {
        name: '后入式',
        desc: [
            '你一把将{name}翻过去，让她跪趴在地上。{hl}的身段在你面前弯成一道美妙的弧线，{pp_adj}{pp_name}从{ph}股缝间若隐若现。你一巴掌拍在她圆润的臀瓣上——{butt}，留下五道指痕，呵斥道：「撅高点！」她哭着照做了，你挺腰从后面一插到底。',
...
function rapePickSexPosition(key, bd, idx) {
    const pos = RAPE_SEX_POSITIONS[key] || RAPE_SEX_POSITIONS.normal;
...
const RAPE_END_DESC = [
    ...
    '一番暴行过后，你喘息着站起身来。她一动不动地躺在地上，只有肩膀在微微耸动。',
...
function _doRapeForeplay(bd, player, callbacks, part) {
    callbacks.clearChoices();
    _sexAddMessage(rapePickForeplay(part), 'narrator');
...
function _doRapeSex(bd, player, callbacks, key) {
    ...
    const pos = rapePickSexPosition(key, bd, idx);
...
function _handleRapeOrgasm(bd, player, callbacks) {
    ...
    callbacks.clearChoices();
    _sexAddMessage('她的花径骤然缩紧，一阵阵剧烈的颤抖从深处传来。', 'narrator');
    s.maleArousal = Math.min(100, s.maleArousal + 20);
    _sexUpdatePanel(s);

    if (s.maleArousal >= 100) {
        callbacks.showChoices([{ text: '继续', action: () => _rapeOrgasmNext1(bd, player, callbacks, isSquirt) }]);
    } else {
        s.orgasmCount++;
        callbacks.showChoices([{ text: '继续', action: () => _rapeOrgasmDone(bd, player, callbacks, isSquirt, false) }]);
    }
}
function _rapeOrgasmNext1(bd, player, callbacks, isSquirt) {
    callbacks.clearChoices();
    _sexAddMessage('她花径的紧缩让你再也无法忍耐，精关一泄如注。', 'narrator');
    callbacks.showChoices([{ text: '继续', action: () => _rapeOrgasmNext2(bd, player, callbacks, isSquirt) }]);
}
function _rapeOrgasmNext2(bd, player, callbacks, isSquirt) {
    callbacks.clearChoices();
    _sexAddMessage('滚烫的阳精尽数喷洒在她花心深处。', 'narrator');
    bd._rapeState.orgasmCount++;
    bd._rapeState.wasInternal = true;
    callbacks.showChoices([{ text: '继续', action: () => _rapeOrgasmDone(bd, player, callbacks, isSquirt, true) }]);
}
function _rapeOrgasmDone(bd, player, callbacks, isSquirt, maleCame) {
    const s = bd._rapeState;
